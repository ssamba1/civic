...
**Resume checkpoint (paused 2026-07-08).** Plan: [2026-07-08-crew-types-invites-dropdowns.md](./2026-07-08-crew-types-invites-dropdowns.md) — the authoritative task spec. This file = execution progress + how to continue.

## Branch & commits
- Branch: `feat/crew-types-invites-dropdowns` (off `main` @ `083a83f`). Working tree CLEAN.
- Remote: SSH (`git@github.com:28gugales-dev/-Social-Impact-.git`). NOT pushed yet.
...
| **2-APPLY — run 031 DDL on prod** | ⏳ **USER ACTION** (auto-mode classifier blocks schema pushes) | — |
| 3 — `src/lib/db/crew-types.ts` (`fetchCityCrewTypes`) | ✅ done, reviewed | `d0cb0c1` |
| 4 — server actions (`createCrewType`, free-text `crewTypeSchema`, `inviteMember`→`userId`) | ⬜ NOT STARTED (next) | — |
| 5 — `MenuSelect` component | ⬜ | — |
| 6 — Role/Team selects → MenuSelect + ERROR_COPY | ⬜ | — |
| 7 — crew dialog MenuSelect + new-type flow | ⬜ | — |
| 8 — invite-by-email in crew roster | ⬜ | — |
| 9 — AI schema+prompt builders (zero-custom = byte-identical) | ⬜ | — |
| 10 — pipeline wiring + `crew_type` type widening | ⬜ | — |
| 11 — full gates + browser QA | ⬜ | — |
| Final — code review + finish branch (merge/PR) | ⬜ | — |

...
/* ------------------------------------------------------------------
   deck.gl layer builders — only the aggregation views (heat / hex) use deck.
   Marker views render DOM <Marker> pins so they can CSS-pulse cheaply.
   ------------------------------------------------------------------ */
...
/**
 * Marker view as deck.gl layers (NOT DOM markers — those collide with the
 * interleaved deck overlay's canvas container). A soft status-colored glow ring
 * + a crisp core dot, copied from report-map.tsx's glow/dots layers. pickable
 * is off everywhere (it's a backdrop) so deck never does ReadPixels picking,
 * which otherwise stalls the GPU.
 */

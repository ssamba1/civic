...
describe("pickPendingNavigation", () => {
  ...
  it("ignores incomplete or errored parts", () => {
    const messages = [
      msg("m1", [
        { type: "tool-navigateTo", toolCallId: "c1", state: "input-available" },
      ]),
      msg("m2", [
        {
          type: "tool-navigateTo",
      ...
      msg("m2", [
        {
          ...
          toolCallId: "c2",
          state: "output-available",
          output: { navigate: "/user/my-reports" },
        },
      ]),
    ];
    expect(pickPendingNavigation(messages as never)?.route).toBe(
      "/user/my-reports",
    );
  });
});

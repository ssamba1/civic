...
describe("buildTimeline", () => {
  ...
  it("rejected report emits created + rejected only", () => {
    ...
    const rejectedTs = new Date(
      Date.parse(BASE_CREATED) + 30 * 60_000,
    ).toISOString();
  ...
  it("closed report emits all lifecycle events including resolved at correct ts", () => {
    ...
    const hours = 12 + 2 * 18;
    const resolvedTs = new Date(
      Date.parse(BASE_CREATED) + hours * 3_600_000,
    ).toISOString();
  ...
  it("interleaves reassigned override events in ascending ts order", () => {
    ...
    for (let i = 1; i < tl.length; i++) {
      expect(Date.parse(tl[i].ts)).toBeGreaterThanOrEqual(
        Date.parse(tl[i - 1].ts),
      );
    }
  });
...
describe("similarNearby", () => {
  ...
  it("counts same-category report within radius", () => {
    const near = makeReport({
      id: "r2",
      category: "pothole",
      location: { lat: 34.2001, lng: -84.1001 },
    });
  ...
  it("counts multiple nearby same-category reports", () => {
    const near1 = makeReport({
      id: "r2",
      category: "pothole",
      location: { lat: 34.2001, lng: -84.1 },
    });
    const near2 = makeReport({
      id: "r3",
      category: "pothole",
      location: { lat: 34.2002, lng: -84.1 },
    });

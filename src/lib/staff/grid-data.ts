...
const logger = createLogger("[staff-grid]");

/**
 * One row of the staff work-order grid.
 *
 * Report-centric (FROM reports), NOT work-order-centric like the Inbox's
 * getWorkOrders(). That difference is deliberate: emergency reports short-circuit
 * the classify pipeline before a work_orders row is created (and merged reports
 * never get one either), so a work-order-first query silently hides the most
 * urgent class of report. Here every classified report appears; the work-order
 * fields are simply null when no work order exists yet — which surfaces the
 * "emergency = dispatched, no priority/cost" reality instead of burying it.
 */
 ...
 * client-side (default Priority desc).
 */
